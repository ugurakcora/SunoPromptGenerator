...
export default function SavedPrompts({ onLoad, refreshTrigger }: SavedPromptsProps) {
    ...
    return (
      <Card className="bg-background/50 border-border/50">
        <CardContent className="p-10 text-center">
          ...
          <p className="text-white/70 font-medium">Henüz kayıtlı prompt yok. Oluşturduğunuz prompt'ları kaydedin!</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-3 sm:space-y-5">
      <h3 className="text-lg sm:text-xl font-bold text-white flex flex-wrap items-center gap-2 sm:gap-3">
        ...
        <span className="px-2 sm:px-3 py-1 sm:py-1.5 bg-background/50 rounded-full text-xs sm:text-sm font-bold text-white border border-border/50">
          {savedPrompts.length}
        </span>
      </h3>
      <div className="grid gap-2 sm:gap-3 max-h-[400px] sm:max-h-[500px] overflow-y-auto pr-1 sm:pr-2">
        {savedPrompts.map(saved => (
          <Card key={saved.id} className="bg-background/50 border-border/50 hover:bg-background/70 transition-all">
            <CardContent className="p-3 sm:p-4 md:p-5">
              <div className="flex flex-col sm:flex-row items-start sm:items-start justify-between gap-2 sm:gap-3">
                <div className="flex-1 min-w-0 w-full sm:w-auto">
                  <h4 className="font-bold text-white mb-1 sm:mb-2 text-sm sm:text-base">{saved.name}</h4>
                  <p className="text-xs sm:text-sm text-white/70 line-clamp-2 mb-2 sm:mb-3 leading-relaxed">{saved.prompt}</p>
        ...
        <DialogContent className="bg-background border-border">
          <DialogHeader>
            <div className="flex items-center gap-3">
              ...
              <DialogTitle className="text-white">Prompt'u Sil</DialogTitle>
            </div>
            <DialogDescription className="text-muted-foreground pt-2">
              Bu işlem geri alınamaz. "{promptToDelete?.name}" adlı prompt kalıcı olarak silinecek.
            </DialogDescription>
          </DialogHeader>
          {promptToDelete && (
            <div className="py-4">
