  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
        ...
        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white break-words">Şarkı Sözleri (Lyrics)</h2>
      </div>
      
      <div className="space-y-1.5 sm:space-y-2">
        <Label className="text-xs sm:text-sm font-bold text-white/70 uppercase tracking-wider">
          Şarkı Sözleri <span className="text-white/50 normal-case font-normal">(Opsiyonel)</span>
        </Label>
        <Textarea
          value={lyrics}
          onChange={(e) => onChange(e.target.value)}
          placeholder="Şarkı sözlerinizi buraya yazın..."
          className="w-full bg-background/50 border-border/50 text-white placeholder:text-muted-foreground min-h-[150px] sm:min-h-[180px] md:min-h-[200px] resize-none text-sm sm:text-base"
          rows={8}
        />
      </div>
    </div>
