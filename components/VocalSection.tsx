...
import { PRIMARY_VOCALS, VOCAL_STYLES, VOCAL_TIMBRES, VOCAL_EFFECTS, FEMALE_VOCAL_RANGES, MALE_VOCAL_RANGES, PromptOptions } from '@/lib/types';
...
export default function VocalSection({ options, onChange }: VocalSectionProps) {
  const showFemaleRange = options.primaryVocal?.includes('Kadın') || options.primaryVocal?.includes('Female');
  const showMaleRange = options.primaryVocal?.includes('Erkek') || options.primaryVocal?.includes('Male');
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
        ...
        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white break-words">4. Vokal (Vocals)</h2>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-5">
        {[
          { key: 'primaryVocal', label: 'Ana Vokal (Primary Vocal)', options: PRIMARY_VOCALS, placeholder: '-- Vokal tipi seçin --' },
          { key: 'vocalStyle', label: 'Vokal Tarzı (Vocal Style)', options: VOCAL_STYLES, placeholder: '-- Tarz seçin --' },
          { key: 'vocalTimbre', label: 'Vokal Timbresi (Vocal Timbre)', options: VOCAL_TIMBRES, placeholder: '-- Renk seçin --' },
          { key: 'vocalEffects', label: 'Vokal Efektleri (Vocal Effects)', options: VOCAL_EFFECTS, placeholder: '-- Efekt seçin --' },
        ...
        {showFemaleRange && (
          <div className="space-y-1.5 sm:space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => onChange({ femaleVocalRange: value || undefined })}
            >
              <SelectTrigger className="w-full bg-background/50 border-border/50 text-white h-10 sm:h-11 md:h-12 text-sm sm:text-base">
        ...
        {showMaleRange && (
          <div className="space-y-1.5 sm:space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => onChange({ maleVocalRange: value || undefined })}
            >
              <SelectTrigger className="w-full bg-background/50 border-border/50 text-white h-10 sm:h-11 md:h-12 text-sm sm:text-base">
                <SelectValue placeholder="-- Aralık seçin --" />
              </SelectTrigger>
